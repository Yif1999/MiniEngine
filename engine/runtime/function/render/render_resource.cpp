...
namespace MiniEngine
{
    ...
    std::shared_ptr<TextureData> RenderResource::loadTextureHDR(std::string file, int desired_channels)
    {
        ...
        texture->m_mip_scenes   = 1;
        texture->m_type         = IMAGE_TYPE::IMAGE_TYPE_2D;

        return texture;
    }

    std::shared_ptr<TextureData> RenderResource::loadTexture(std::string file, bool is_srgb)
    {
        std::shared_ptr<AssetManager> asset_manager = g_runtime_global_context.m_asset_manager;
        ASSERT(asset_manager);
    ...
    RenderMeshData RenderResource::loadMeshData(const MeshSourceDesc& source, AxisAlignedBox& bounding_box)
    {
        ...
        else if (std::filesystem::path(source.m_mesh_file).extension() == ".json")
        {
            ...
            // vertex buffer
            size_t vertex_size                     = bind_data->vertices.size() * sizeof(MeshVertexDataDefinition);
            ...
            for (size_t i = 0; i < bind_data->vertices.size(); i++)
            {
                vertex[i].x  = bind_data->vertices[i].px;
                vertex[i].y  = bind_data->vertices[i].py;
                vertex[i].z  = bind_data->vertices[i].pz;
                vertex[i].nx = bind_data->vertices[i].nx;
                vertex[i].ny = bind_data->vertices[i].ny;
                vertex[i].nz = bind_data->vertices[i].nz;
                vertex[i].tx = bind_data->vertices[i].tx;
                vertex[i].ty = bind_data->vertices[i].ty;
                vertex[i].tz = bind_data->vertices[i].tz;
                vertex[i].u  = bind_data->vertices[i].u;
                vertex[i].v  = bind_data->vertices[i].v;

                bounding_box.merge(Vector3(vertex[i].x, vertex[i].y, vertex[i].z));
            }

            // index buffer
            size_t index_size                     = bind_data->indices.size() * sizeof(uint16_t);
            ...
            for (size_t i = 0; i < bind_data->indices.size(); i++)
            {
                index[i] = static_cast<uint16_t>(bind_data->indices[i]);
            }
        }
